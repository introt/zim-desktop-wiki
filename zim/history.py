...
The main class is :class:`History`. Also there is a specialized class
:class:`HistoryPath` which extends :class:`Path` with history information.
'''
...
class HistoryPath(Path):
	...
	:ivar is_first: ``True`` when this is the first path in the history
	:ivar is_last: ``True`` when this is the last path in the history
	'''
...
class HistoryList(list):
	'''A list of :class:`HistoryPath`s which takes care of serialization
...
class History(SignalEmitter):
	'''History class, keeps track of a list of :class:`HistoryPath` objects.
	...
	:ivar notebook: the :class:`Notebook`
	:ivar uistate: the :class:`ConfigDict` used to store the history

	:signal: ``changed ()``: emitted when the path list changed
	'''
	...
	def __init__(self, notebook, uistate=None):
		...
		:param uistate: :class:`SectionedConfigDict` to store the history (history
	...
	def append(self, path):
		...
		:param path: :class:`Path` for the current page
	...
	def get_current(self):
		...
		:returns: a :class:`HistoryPath` object
		'''
		if self._history:
	...
	def set_current(self, path):
		...
		:param path: a :class:`HistoryPath` object
	...
	def get_previous(self):
		...
		:returns: a :class:`HistoryPath` object or ``None`` if current is
	...
	def get_child(self, path):
		...
		:class:`Path` object. Used by the keybinding for navigating to child
		pages.

		:param path: a :class:`Path` object
		:returns: a :class:`HistoryPath` or :class:`Path` object or ``None``
		'''
		for list in (self._history, self._recent):
	...
	def get_grandchild(self, path):
		...
		:param path: a :class:`Path` object
		:returns: a :class:`HistoryPath` object or ``None``
		'''
		child = path
		for list in (self._history, self._recent):
			for p in reversed(list):
	...
	def get_state(self, path):
		...
		:param path: a :class:`Path` object
		:returns: a tuple of cursor and scroll position for ``path``
			or ``(None, None)``
		'''
		for list in (self._history, self._recent):
			for record in reversed(list):
	...
	def get_history(self):
		...
		:returns: yields :class:`HistoryPath` objects
		'''
	...
	def get_recent(self):
		...
		:returns: yields :class:`RecentPath` objects
		'''
		# Generator to avoid external acces to the list
