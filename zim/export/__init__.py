...
The main API for exporting from the application is the :class:`Exporter`
object. There are subclasses of Exporter to export to multiple files,
to a single file or to a MHTML file.

To configure the exporter object an additional :class:`ExportLayout` object
is used. This layout defines the exact mapping of pages to files to
be used.

To specific the pages to export a subclass of the :class:`PageSelection` class
is used. There are selection classes to export the whole notebook or
to export a single page.

The :class:`ExportTemplateContext` object defines specific template parameters
for exporting. See also :class:`zim.templates` for template parsing and
processing classes.
...
def build_notebook_exporter(dir, format, template, **opts):
	'''Returns an :class:`Exporter` that is suitable for exporting a whole
...
def build_page_exporter(file, format, template, page, **opts):
	'''Returns an :class:`Exporter` that is suitable for exporting a page with
...
def build_single_file_exporter(file, format, template, namespace=None, **opts):
	'''Returns an :class:`Exporter` that is suitable for exporting a set of
